...
void TFrmMain::SaveToFile(const char* pszFileName)
{
    ...
    iFileHandle = FileOpen(NewFileName, fmOpenRead | fmOpenWrite); //´ò¿ªÎÄ¼þ

    if ((stream = fopen(CurrentOpenFile.c_str(), "r+"))
            == NULL)
    {
        ShowMessage("´ò¿ªÎÄ¼þ³ö´í");
        return;
    }
...
void __fastcall TFrmMain::N4Click(TObject* Sender)
{
    ...
    */
    iFileHandle = FileOpen(CurrentOpenFile, fmOpenRead | fmOpenWrite); //´ò¿ªÎÄ¼þ
